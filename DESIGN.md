...
CPS → internal types must be thorough, tested against real fixtures, and documented per format under `docs/reference/<format>/`. Import is the hard problem we invest in.
...
Radio caps, column names, wire strings, and profile limits apply only in `core/import-export/formats/` and `docs/reference/<format>/`. The library model, domain mutations, validation, and library CRUD UI do not embed `OPENGD77_`* constants or format-specific cardinality.
