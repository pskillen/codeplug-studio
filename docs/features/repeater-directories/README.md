...
| **Check and update current channel** | Channel editor **Identity** → _Check ukrepeater.net_ / _Check IRTS_ / _Check RepeaterBook_ / _Check BrandMeister repeater_; **Mode settings → DMR** → _Check BrandMeister talk groups & RX list_ | Repeater field diff (UK, IRTS, RepeaterBook, or BM repeater); separate BM button for RX group list sync ([#65](https://github.com/pskillen/codeplug-studio/issues/65), [#1262](https://github.com/pskillen/codeplug-studio/issues/1262))                                                                                                                                                                                                                    |
...
| Verify / update | `ChannelDirectoryVerifyActions.tsx`, `BrandmeisterRxListSyncAction.tsx`, `RepeaterListingUpdateDialog.tsx`, `BrandmeisterRxGroupListSyncDialog.tsx` | `channelDiff.ts` patch; RX list sync ([#65](https://github.com/pskillen/codeplug-studio/issues/65)); RepeaterBook verify ([#274](https://github.com/pskillen/codeplug-studio/issues/274)); placement [#1262](https://github.com/pskillen/codeplug-studio/issues/1262); Apply & save / Apply only + New-channel entry point [#1265](https://github.com/pskillen/codeplug-studio/issues/1265) |
...
### Apply & save vs Apply only ([#1265](https://github.com/pskillen/codeplug-studio/issues/1265))

`RepeaterListingUpdateDialog` offers two apply actions, both driven by the same diff/patch logic:

- **Apply & save** persists the patch immediately (`persistence.putChannel(patched, channel.revision)`) and closes the dialog. This is the existing one-click "check a saved channel → diff → apply" workflow, unchanged by this rework — operators rely on it being fast, and it stays exactly as it was.
- **Apply only** hands the patched `Channel` back to the caller and writes nothing. It exists for the channel editor's **New channel** screen: there's no existing channel row to persist against yet, so the caller (`ChannelEditor.tsx`) fans the patch out into its own in-progress form state instead, letting the operator keep filling in the rest of the channel before the first save.

The channel editor's Identity panel now renders [`ChannelDirectoryVerifyActions`](../../../src/app/components/repeaters/ChannelDirectoryVerifyActions.md) on **New channel** as well as on saved channels — previously it was hidden entirely until the channel had been saved once, forcing manual entry even though the lookup-by-callsign workflow was otherwise fully built. The dialog's `mode` prop switches its copy accordingly: `'verify'` ("Check ukrepeater.net…") on a saved channel, `'lookup'` ("Look up on ukrepeater.net…", title "Import from directory") on New channel.
...
| `RepeaterListingUpdateDialog.tsx`       | Directory comparison `ModalShell` (diff table, Apply & save / Apply only, `mode: 'verify' \| 'lookup'`) — sidecar [`RepeaterListingUpdateDialog.md`](../../../src/app/components/repeaters/RepeaterListingUpdateDialog.md)                                        |
| `ChannelDirectoryVerifyActions.tsx`     | Channel editor Identity verify/lookup (UK, IRTS, RepeaterBook, optional BrandMeister repeater) — renders on New channel and saved channels — sidecar [`ChannelDirectoryVerifyActions.md`](../../../src/app/components/repeaters/ChannelDirectoryVerifyActions.md) |
| `BrandmeisterRxListSyncAction.tsx`      | Channel editor DMR RX-list sync — sidecar [`BrandmeisterRxListSyncAction.md`](../../../src/app/components/repeaters/BrandmeisterRxListSyncAction.md)                                                                                                              |
