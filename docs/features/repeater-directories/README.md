...
| RepeaterBook client                 | Shipped  | Export API (`export.php` / `exportROW.php`); per-user `rbuapp_` token ([#274](https://github.com/pskillen/codeplug-studio/issues/274))                                                                                                                                                                                             |
...
| RepeaterBook search UI              | Shipped  | NA/ROW region; state/country + callsign; band/mode/geometry filters ([#274](https://github.com/pskillen/codeplug-studio/issues/274))                                                                                                                                                                                               |
...
| RepeaterBook            | `searchRepeaterBook` / `repeaterbook/queryRouter.ts` | NA: state/country/callsign; ROW: country/callsign; band/mode/geometry client-side                                          | Export JSON; `rbuapp_` token; approved User-Agent    |
...
- ukrepeater.net and BrandMeister allow browser CORS; IRTS uses a same-origin Pages Function proxy (see [IRTS reference](../../reference/irts/README.md)); RepeaterBook uses browser-direct fetch with per-user token (see [RepeaterBook reference](../../reference/repeaterbook/README.md)). Failures surface as `RepeaterDirectoryError` in the UI.
