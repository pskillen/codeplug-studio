...
| `mobileCard` (v2 `DataTable` only) | Replaces per-column grid cells with a card render at the mobile breakpoint — see [DataTable.md](../../../src/app/components/v2/DataTable.md#mobile-card-rows). Channels list uses an explicit **Cards** layout instead ([#1205](https://github.com/pskillen/codeplug-studio/issues/1205)). |
