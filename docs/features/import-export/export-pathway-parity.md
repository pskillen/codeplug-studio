...
| --- | -------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Channel wire name**                        | Same base compose (callsign/name per export name mode), abbreviation policy (strategy-when-over-limit, not unconditional substitution), shortening trigger, override precedence, multi-mode / multi-TG suffix handling | `src/core/domain/channelNaming.ts` · `src/core/import-export/channelExpansion/exportWireNames.ts` · non-expansion channel rows resolve via `resolveWireNames()` / `resolveWireNamesFromOptions()` (`src/core/services/resolveWireNamesCore.ts`) on every pathway (preview, radio-io, and CSV serialisers alike). m×n / multi-mode / timeslot-clone expansion rows still compose per-format (`anytoneChannelWireName`, `expandOpenGd77ChannelWireRows`, DM32/NeonPlug channel expansion) — a documented, not-yet-closed gap, tracked as before |
...
| 6   | **Talk group / contact naming**              | Same names, same FK resolution strategy per format family; timeslot-clone naming                                                                                                                                       | `talkGroupWireNames.ts` · `multiTalkGroupWireName.ts` · `talkGroupTimeslotClones.ts` · talk group/contact/RX-group-list/zone/scan-list names resolve via `resolveWireNames()` / `resolveWireNamesFromOptions()` on every pathway (preview, radio-io, and CSV — Anytone, OpenGD77, DM32, NeonPlug); each entity kind resolves against its own independent reserved-name set (no cross-kind disambiguation, e.g. a zone and a scan list may legitimately share a wire name)                                                                     |
...
- Zone / scan-list / talk-group / contact / RX-group-list wire naming — Anytone, OpenGD77, DM32,
  and NeonPlug CSV serialisers now resolve these kinds via `resolveWireNamesFromOptions()`, the
  same policy engine preview and radio-io already used (export wire-preview rework phase 5).
