...
- **Pass grid filters** — client-only, applied to the already-computed pass list (no Worker re-run): a min-elevation filter and a satellite multi-select-with-search filter (`src/app/routes/tracking/SatelliteFilter.tsx`). The look-ahead window (default 12h) is a user-adjustable control on the dashboard, debounced 300ms before triggering a new Worker pass-prediction sweep. Large pass lists use windowed row virtualization on the v2 `DataTable`.
