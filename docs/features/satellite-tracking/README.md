...
| SatNOGS transmitters proxy + merge | Shipped     | [#864](https://github.com/pskillen/codeplug-studio/issues/864) — proxy + `noradId`-keyed merge only; data-availability, not display (grid columns remain a deliberate follow-up, see Out of scope)                                                                                |
...
| Satellite detail page              | In progress | [#1002](https://github.com/pskillen/codeplug-studio/issues/1002) — route, static detail panel, and future/past pass lists shipped ([#1003](https://github.com/pskillen/codeplug-studio/issues/1003)); live position, visible-horizon footprint, and orbit-trail map still to come |
...
- **SatNOGS enrichment vs. TLE source** — SatNOGS answers "what does this satellite transmit / is it alive" (`SatelliteEnrichmentSource`), a different provenance question from `SatelliteSource` ("where did this TLE come from" — CelesTrak/AMSAT). Enrichment is fetched and merged live per session, keyed by NORAD id, and is **not** persisted as part of the `Satellite` model — see [SatNOGS reference](../../reference/remote-directories/satnogs/README.md).
