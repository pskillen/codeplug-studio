...
| Restore to radio                              | Shipped (D890) | [#1141](https://github.com/pskillen/codeplug-studio/issues/1141) — AT-D890UV `restoreFromBackup` (hardware verify pending). Other radios stay inspect-only until their restore PR. Serial mismatch is a hard refuse. |
| Per-radio restorable vs inspect-only maps     | Partial        | AT-D890UV table: [at-d890uv/backup-restore.md](../../reference/radios/anytone/at-d890uv/backup-restore.md). Other families still follow this contract until their restore PR.                                        |
