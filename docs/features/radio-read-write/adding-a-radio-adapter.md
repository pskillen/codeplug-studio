...
| 2026-07-24 | DM-32UV #685    | **Contact-bank Read:** clone download **skips** V-frame `0x0F` address-book blocks (not needed for hydration stash; L01 span is huge/often empty). Count-based fold helper retained on protocol for a future contacts Read — never walk start→end (was “block *n* of 3464”).                                                                                                                                                                  |
| 2026-07-24 | OpenGD77 #691   | **Full modelled Write encode:** hydration merge always replaces DMR contact, zone, RX group, and channel banks from `RadioWriteProjection` — no RMW of prior Read bytes inside those records. Unmodelled channel offsets (TOT, flags, APRS index, alias, squelch, lat/lon, DMR-ID override) encode to firmware-safe defaults. Settings / APRS / DTMF / VFO / additional settings stay Read-retained via full-image dirty-sector upload.    |
