...
| 2026-07-24 | DM-32UV #685    | **Full entity Write encode:** channel records encode every byte from projection + NeonPlug defaults (unmodelled emergency/encryption/PTT/signaling); unused channel slots and TX-contact entries clear to sentinels; `forbidTransmit` → `rxOnly` + aviation TX `FF×4`; digital-contact bank clears all entry slots across the V-frame span on shrink. Org banks (zones, scan, TG, RX) already full-replace; shrink tests added.            |
