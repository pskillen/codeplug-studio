...
| 2026-07-25 | AT-D890UV #717   | **Write fidelity:** channel frequencies are BCD-as-hex digit strings in **10 Hz** units (not `hz.toString(16)`). Empty projection `radioIds` must **retain** hydrated RadioIdSet/Data (do not `clearBitmap`). Talkgroup **index stride** is `0xc8` (`TalkgroupDataOffset`); `0xd0` is only the 16-aligned sparse R/W length — do not use align-padded length as address pitch. Hardware re-check pending before calling Write safe.                                                                              |
