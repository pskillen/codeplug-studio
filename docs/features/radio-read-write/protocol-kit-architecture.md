...
| **PROGRAM→QX**                  | ASCII `PROGRAM` → `QX`+`0x06`; echo-strip; `0x10` R/W + checksum   | **Shipped** `#641` (`programQx.ts`) — `BlockCodec` + echo-strip helpers; not Mini / not Anytone DMR u32 | CHIRP `anytone778uv.py`; tier-3: [retevis/rt95](../../reference/radios/retevis/rt95/README.md) ([#642](https://github.com/pskillen/codeplug-studio/issues/642))                                                                                                |
| **Anytone DMR R/W**             | `PROGRAM`→`QX`; **921600**; ASCII R/W + **u32 BE**; sparse regions | Shipped `#646` (`anytoneDmrRw.ts`) — `BlockCodec` + handshake helpers; not RT95 `programQx` / Mini      | anytone-cps `SerialDevice` + `D890_MAP`; tier-3: [anytone/at-d890uv](../../reference/radios/anytone/at-d890uv/README.md) ([#647](https://github.com/pskillen/codeplug-studio/issues/647)); epic [#645](https://github.com/pskillen/codeplug-studio/issues/645) |
...
**Surface choice:** PROGRAM+R/W, **PROGRAM→QX**, and **Anytone DMR R/W** all export `BlockCodec` (addr/length R/W) plus session helpers. V-probe has no write and is not block-shaped; OpenGD77 needs mem-region codes, multi-step flash writes, and dual ACK semantics — both are dedicated sibling modules under `kit/codecs/`. **PROGRAM→QX** (RT95 / 778 family) is shipped as `programQx.ts` ([#641](https://github.com/pskillen/codeplug-studio/issues/641)) — echo-strip transport, u16 addresses, 16-byte blocks, checksum frames. **Anytone DMR R/W** (D890UV / D878UVII) shares PROGRAM→QX enter DNA but uses **921600**, **u32 BE** addresses, 8-bit checksums, and sparse multi‑MB maps — shipped as `anytoneDmrRw.ts` ([#646](https://github.com/pskillen/codeplug-studio/issues/646)); do not reuse RT95 u16/`programQx` framing ([#647](https://github.com/pskillen/codeplug-studio/issues/647) docs).
...
src/integrations/radio-io/
  ...
  kit/
    ...
    codecs/
      ...
      programQx.ts        # shipped #641 (BlockCodec + echo-strip; u16 + checksum)
      anytoneDmrRw.ts     # shipped #646 (BlockCodec + PROGRAM→QX handshake; u32 + checksum)
