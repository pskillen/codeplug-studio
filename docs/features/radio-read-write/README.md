...
| First radio path (UV-5R Mini)                 | Shipped (adapter) | [#617](https://github.com/pskillen/codeplug-studio/issues/617) — `radios/uv5r-mini/` + registry; Write via FormatBuild + `assemble`; Read → `cpsWireHydration` `radio-clone` (no library import). UI [#618](https://github.com/pskillen/codeplug-studio/issues/618)                                   |
