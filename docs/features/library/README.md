...
- **Card details** — **Show/hide details** toggles optional field rows on cards independently of table **Show/hide cols** (separate `localStorage` keys). Table **Show/hide cols** includes **Callsign** (`__callsign__`); card details do not — callsign already sits in the card header.
