...
- **Table Name column** — merges **Name**, **callsign + Band** pills (second line), and a joined **Mode** pill group (third line) into one non-hideable column sized to content (table scrolls horizontally when modes are long). There is no separate Callsign/Band/Mode column in the table.
...
- **Layout (mk2 E1, [#941](https://github.com/pskillen/codeplug-studio/issues/941), Identity polish [#1209](https://github.com/pskillen/codeplug-studio/issues/1209), verify placement [#1262](https://github.com/pskillen/codeplug-studio/issues/1262)):** `EditorHeader` (back crumb, title, subtitle) + scroll `Panel` sections + `StickyFooter` (Cancel/Save, dirty status). Order: Identity (callsign + directory verify on edit, name + abbreviation, comment; `ChannelModesField` multi-select) → Frequency → Mode settings (stacked blocks; DMR includes BrandMeister RX-list sync on edit) → Location → Zones (edit) → Scanning → APRS. Product-only Scanning/APRS panels are not in mk2 frames but remain restyled in the same chrome.
- **Identity:** callsign on its own row with [`ChannelDirectoryVerifyActions`](../../../src/app/components/repeaters/ChannelDirectoryVerifyActions.tsx) below it on saved channels (ukrepeater.net, IRTS, RepeaterBook, optional BrandMeister repeater; disabled when callsign is empty). Name and abbreviation together; comment below. `ChannelWireNameExamples` in a second column on desktop; collapsed **Name examples** on mobile. Mode chips use `modeColor` / `ModePill` fills.
...
- **Transmit / TX permit (Frequency):** `ForbidTransmitSegment` (**RX only** = receive-only) and `TxPermitSegment` (**Busy lock** vs **Permit always** while the frequency is in use) — moved from Scanning in [#1209](https://github.com/pskillen/codeplug-studio/issues/1209).
- **Power (Frequency):** `PercentLevelSlider` stores vendor-neutral percent (`null` = radio default). `PowerLadderHints` beside the slider on desktop; collapsed **Power examples** on mobile ([#414](https://github.com/pskillen/codeplug-studio/issues/414), [#1209](https://github.com/pskillen/codeplug-studio/issues/1209)). Informational only.
- **Scanning:** scan inclusion + optional `scanListId` FK only (TX controls are under Frequency).
...
- Component sidecars: `ChannelModesField.md`, `EditorHeader.md`, `StickyFooter.md`, and siblings under `src/app/components/channels/`; directory verify sidecars under `src/app/components/repeaters/`.
