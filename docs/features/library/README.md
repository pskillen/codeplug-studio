...
- **Card details** — **Show/hide details** toggles optional field rows on cards independently of table **Show/hide cols** (separate `localStorage` keys). Card **Band** / **Mode** remain individual field rows there.
- **Table Name column** — merges **Name**, **callsign + Band** pills (second line), and **Mode** pills (third line) into one non-hideable column; there is no separate Callsign/Band/Mode column in the table (cards keep separate rows for all three, unaffected).
- **Table Tone column** — optional, hidden by default, placed after **Frequency**. One analog mode profile shows `rxTone/txTone`; more than one shows one `MODE: rxTone/txTone` line per analog profile (`None` for unset tones). Digital-only channels show `—`.
...
- **Layout (mk2 E1, [#941](https://github.com/pskillen/codeplug-studio/issues/941), Identity polish [#1209](https://github.com/pskillen/codeplug-studio/issues/1209)):** `EditorHeader` (back crumb, title, subtitle) + scroll `Panel` sections + `StickyFooter` (Cancel/Save, dirty status). Order: Identity (callsign, name + abbreviation, comment; `ChannelModesField` multi-select) → Frequency → Mode settings (stacked blocks) → Location → Zones (edit) → Scanning → APRS → Repeater info (edit). Product-only Scanning/APRS/Repeater panels are not in mk2 frames but remain restyled in the same chrome.
- **Identity:** callsign on its own row; name and abbreviation together; comment below. `ChannelWireNameExamples` in a second column on desktop; collapsed **Name examples** on mobile. Mode chips use `modeColor` / `ModePill` fills.
