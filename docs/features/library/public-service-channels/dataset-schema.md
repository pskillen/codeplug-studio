...
Copy the header from the template. A worked **fictional** example is `scripts/public-services/example.csv` (country `ZZ`, `example.org` URLs) — copy its *shape*, never its rows.
