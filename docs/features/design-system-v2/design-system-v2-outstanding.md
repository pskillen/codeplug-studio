...
**Retained `ui/*` holdouts** after [#927](https://github.com/pskillen/codeplug-studio/issues/927): tracked in [#1097](https://github.com/pskillen/codeplug-studio/issues/1097).

## Debt

| Item                                               | Severity      | Notes                                                                                                                                                                                                                                                                                                                                                                                         |
| -------------------------------------------------- | ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Domain BandPill / ModePill in membership rows      | Low           | Some membership/directories rows still use v1 `BandPill` / `ModePill` — re-skin to v2 `Pill` ([#1097](https://github.com/pskillen/codeplug-studio/issues/1097))                                                                                                                                                                                                                               |
...
| `DataTable` v2 `storedOrder` not built             | Low           | Capability inventory flags it as speculative — "API + styleguide only, no current product call site." Add against a real consumer if one emerges                                                                                                                                                                                                                                              |
...
| Retained `ui/*` shells and outliers                | Low           | `FormPage`, `GradientSegmentedControl`, `SelectedItemList`, `SplitButton`, debug `Page*` — [#1097](https://github.com/pskillen/codeplug-studio/issues/1097)                                                                                                                                                                                                                                   |
...
## Closed during v1 styleguide retire (#927)

- v1 `DataTable`, `EmptyState`, `SoftWarning`, `YamlFileDropzone`, `ShuttleList`, `BadgeCard`, and related stubs deleted
- Product DataTables migrated to v2 (`BuildsListPage` list mode, flat-memory scan, wire bulk edit, debug tables)
- Reference index restyled off `ListPage` onto v2 `Panel` chrome
- `UnsavedChangesModal`, `SelectedItemDragHandle`, `PercentLevelSlider` promoted to `components/v2`
- `/styleguide` promoted to v2 hub; `/styleguide/v2/*` redirects; v1 demo pages removed
- Retained holdouts filed as [#1097](https://github.com/pskillen/codeplug-studio/issues/1097)
