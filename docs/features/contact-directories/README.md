...
Radios with trait `separateDigitalIdList` expose separate **library contact** and **RadioID directory** banks on Write. Overlap rule: when both contribute, directory rows whose `digitalId` matches a library digital contact are skipped (library wins).
