...
| ----------------- | -------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **List**          | `WirePreviewDataTable` (v2 `DataTable`) on entity routes       | Browse, search; inline **Skip** / **Force export**; **`reorderMode`** + drag/order column when reorder config present; **`bulkReorder`** (multi-select, drag, Move toolbar) on build Zones ([#590](https://github.com/pskillen/codeplug-studio/issues/590)); row click opens modal |
...
| **Modal**         | `WirePreviewOverrideModal` (`ModalShell` + v2 `OverrideField`) | Wire name, skip, force-include; zone rows use **Export / Members / Scan** tabs ([#472](https://github.com/pskillen/codeplug-studio/issues/472)); other kinds stay a single stack                                                                                                   |
