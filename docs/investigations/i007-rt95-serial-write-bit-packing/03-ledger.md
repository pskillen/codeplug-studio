...
| `R/studio-encode` | `harness/encode-studio-bytes.mts` via `npx vite-node --script`                                                                                                 | repo 2026-08-24                      | High+none+FM → `20 20`; plus → `60 20`; tx_off → `20 a0`; TSQL `03`; DTCS both `0c`; invert TX byte17 `40`; simplex offset BCD `00206514`; +600 kHz `00000600` | Pre-bit-pack Studio encode. Bit fields superseded by later code; **BCD LE bytes remain the live freq packing**. |
| `R/chirp-bbcd`    | inline Python `chirp.bitwise` `bbcd freq[4]` / `offset[4]`                                                                                                     | `PYTHONPATH=$CHIRP_ROOT`; 2026-08-25 | 146.52 MHz → `14 65 20 00`; 7.6 MHz offset → `00 76 00 00`; 430.850 MHz → `43 08 50 00`. Studio LE of those: `00 20 65 14` / `00 00 76 00` / `00 50 08 43`     | CHIRP `bbcd` is MSD-first. Studio `bcd.ts` is LSD-first. Radio `bbcd` of Studio 7.6 MHz LE is 0.076 MHz.        |

## Captures / dumps

| id                  | Artefact                                                               | Source                                | Why it matters                                                                                                         |
| ------------------- | ---------------------------------------------------------------------- | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `O/2026-08-25-rt95` | Operator LCD after Studio Web Serial Write of latest packing-fix build | RT95 VOX; GB3GL UHF +7.6 MHz expected | CTCSS correct; offset **+0.076 MHz**; PTT **null freq**; all memories RX **136.000 or 490.000 MHz** (band-edge coerce) | Bit/tone packing reached the radio. Frequency/offset BCD did not. |
