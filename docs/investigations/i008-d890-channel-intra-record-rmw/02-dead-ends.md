...
| Fixing omitted DTO fields one-by-one (always write `0x1c`, always write DMR MODE) removes the class of bug.         | W4, W9                    | Those patches stop _those_ fields leaking. Every remaining unmodelled-in-record byte still copies the previous occupant. The next sticky field is the same engine.                                                                                         |
