...
Staging and prod Android builds also produce a signed `.aab` (workflow artifact only, never a Release asset). A GitHub pre-release ensures the bundle on Play Open testing; a full release ensures Production. `main` / `dev` pushes are APK-only. See [play-store-publish.md](../features/android-app/play-store-publish.md).
