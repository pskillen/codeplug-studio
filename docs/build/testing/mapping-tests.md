...
Wire-format column detail: `docs/reference/export-formats/<format>/`. Strategy docs cite **outcomes** (golden snapshots, lossy fields), not every column.
...
| **Export mapping (OpenGD77)**    | Constructed library + build → CPS CSV set | `exportGolden.test.ts`, `serialise.test.ts`           | Shipped — golden Channels/Zones/Contacts/TG_Lists; multi-mode, shortening, CPS-safe defaults ([#92](https://github.com/pskillen/codeplug-studio/issues/92)) |
...
| -------- | ------------- | ----------------------------------------------------- | ---------------- |
| OpenGD77 | Planned       | Shipped (`exportGolden.test.ts`, `serialise.test.ts`) | Optional         |
