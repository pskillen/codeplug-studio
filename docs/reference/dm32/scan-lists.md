...
1. Emit a `Scan.csv` row named after the zone wire name, shortened to **`scanListNameLimit` (13)** when **Shorten long names** is on (general zone/channel `nameLimit` remains 16; [#485](https://github.com/pskillen/codeplug-studio/issues/485))
...
4. Set channel `Scan List` FK on the scan **carrier** channel only (members keep manual assignment) — FK uses the **same** ≤13 Scan Name string
