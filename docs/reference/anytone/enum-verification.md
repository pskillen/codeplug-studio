...
No enum columns — name/FK columns only. Populated body rows confirm a **5-column** schema (`No.`, `Zone Name`, `Zone Channel Member`, `A Channel`, `Scan Channel `) distinct from `DMRZone.CSV`. See [am-air.md](am-air.md). Export partition tracked in [#316](https://github.com/pskillen/codeplug-studio/issues/316).
