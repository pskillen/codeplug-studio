...
Enable TG expansion (`ExportOptions.expandRxGroupLists`) when the operator wants **one memory per RX-list member** on export. DM32 and NeonPlug DM32UV default **on**; Anytone defaults **on** but lean export (native RGL on each row) is still available when off.
