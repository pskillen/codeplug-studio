...
5. Progress copy is **Restore**, not “Writing codeplug.” Write-codeplug still requires a persisted bag (`hydrationRequiredForWrite: true`) **and** an in-session content pre-write read; Restore does **not** call `upload()`, assemble, or that pre-write read.
