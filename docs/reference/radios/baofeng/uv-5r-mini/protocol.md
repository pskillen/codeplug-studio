...
4. For **Write** upload: in-session pre-write read of all `MEM_*` blocks (progress **Pre-write read**), overlay modelled channels onto that live packed image, then upload handshake (NeonPlug trailer `0x01`) → write all `MEM_*` regions ([settings.md](settings.md)). Project stash still required until phase 07; overlay uses the live prior, not the bag alone. Hardware verify pending.
