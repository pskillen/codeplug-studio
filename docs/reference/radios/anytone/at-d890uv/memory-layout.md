...
| `RadioIdSet`                 | `0x3482c40`  | Bitmap `0x20`                                             | Radio ID occupancy. Bit **set** → occupied. Write **encode prior** is the in-session download cache ([#875](https://github.com/pskillen/codeplug-studio/issues/875)) — an empty projection **retains** live occupancy / records. Do **not** treat assemble-from-`0xff` zeroing of this bitmap as the product contract ([#1129](https://github.com/pskillen/codeplug-studio/issues/1129) parked). Shares erase unit `0x3480000` with `ChannelSet` / `ZoneSet` / `ZoneHide`.                                                                                                               |
| `RadioIdData`                | `0x3680000`  | Stride `0x40`; length `0x40`                              | Radio ID records                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `ScanListSet`                | `0x3482c60`  | Bitmap `0x20`                                             | Scan-list occupancy                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `ScanListData`               | `0x2100000`  | Stride `0x200`; used through `0xF9` (encode `0xfa` today) | Scan-list records — **100** member u16s at `+0x30`…`+0xF7` (0-based global channel indices); `revert_channel` @ `+0xF8`; priority @ `+0x2` / `+0x4`: `0xffff` = Off — [scan-list-record.md](scan-list-record.md). Full stride zero-fill past `0xF9` deferred (F4). **Product encode / dump / verify Read:** `100 × 0x200` from `SCAN_LISTS_MAX`. Occupancy bitmap bits above slot 99 may be set on hardware, but flat records from `0x2100000` are not valid for those slots — anytone-cps's flat 250-list map is wrong ([#845](https://github.com/pskillen/codeplug-studio/issues/845)) |
...
| `ReceiveGroupSet`            | `0x3701510`  | Bitmap `0x20`                                             | RX-group occupancy — `RX_GROUP_SET_BYTES` ([#845](https://github.com/pskillen/codeplug-studio/issues/845))                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `ReceiveGroupData`           | `0x3780000`  | Stride `0x200`; length `0x120`                            | Receive-group lists — [receive-group-record.md](receive-group-record.md) (members = talkgroup bank slot indices). **250 × 0x200** from `RX_GROUP_LISTS_MAX` ([#845](https://github.com/pskillen/codeplug-studio/issues/845))                                                                                                                                                                                                                                                                                                                                                             |
...
On **Write**, Studio encodes the modelled overlay onto `cacheToMemoryMap` of this PROGRAM session download cache ([#875](https://github.com/pskillen/codeplug-studio/issues/875)) — not a persisted hydration bag and not a virgin `0xff` `MemoryMap`. Sparse erase-unit RMW ([#768](https://github.com/pskillen/codeplug-studio/issues/768)) then **fresh-reads** each touched `0x40000` unit from the connected radio and re-stages non-`0xff` bytes unchanged **outside** that modelled overlay — including optional settings and alarm spans that share units with modelled banks. Studio does **not** re-derive those kept bytes from the library build. Identity is the radio on the cable this session ([#1276](https://github.com/pskillen/codeplug-studio/issues/1276)); Restore still refuses zip vs live serial mismatch.
...
**Encode prior:** `encodeAtD890ProjectionOntoImage` runs on `cacheToMemoryMap(session download cache)` from this PROGRAM `download()`. Empty cache refuses Write (no `0xff` fallback). Occupied `ZoneHide` bits are cleared unless hide is modelled; zero visible zones are refused ([#1125](https://github.com/pskillen/codeplug-studio/issues/1125)). Channel `0x80` bodies are **whole records** from the projection plus write-defaults ([channel-record.md](channel-record.md)); the image prior is for other regions / ChannelSet bits ≥ 4000, not the previous occupant of a reused memory index ([#1273](https://github.com/pskillen/codeplug-studio/issues/1273)).

**Sparse RMW flow (upload):** compute touched erase units from modelled write addresses → fresh-read each unit from the connected radio → overlay modelled chunks → stage only non-`0xff` 16-byte blocks **excluding** per-unit flash sector-management markers at `+0x3fbf0` / `+0x3fff0` → transmit filtered chunks → `END` (bare `0x06` ACK; no further wire command). There is **no** separate commit command — see [flash-sectors.md](flash-sectors.md). RMW preserves co-residents **outside** the modelled overlay; it is not a substitute for the in-session encode prior.
...
The same `+0x40000` offset appears when Studio incorrectly transmitted flash sector-management markers: writes landed in a shadow window at `address + 0x40000` instead of on the live bank. That diversion is **probably** the same A/B sector mechanism as this alias stride — **inference, not proven**. Never write the markers; see [flash-sectors.md](flash-sectors.md) and investigation [i001](../../../investigations/i001-d890-write-commit-failure/README.md).
...
Measured in `ChannelData` 2026-07-27 (`/debug/d890-erase-probe`, `ID890UV`): **`0x40000` (256 kB), aligned**. Writing into a unit can erase the whole unit; only bytes re-staged in the same PROGRAM session survive. There is no separate erase/commit opcode — see [flash-sectors.md](flash-sectors.md).
...
| RadioIdSet      | `0x20`  | Bit **set** → radio ID occupied. Live-cache encode retains this when the projection has no IDs; assemble-from-`0xff` zeros are **not** the Write contract ([#1129](https://github.com/pskillen/codeplug-studio/issues/1129)). |
...
Dump / write-verify region sizes for `ScanListData`, `ReceiveGroupSet`, and `ReceiveGroupData` derive from `AT_D890UV_LIMITS` / `AT_D890_LIMITS` — same source as encode ([#845](https://github.com/pskillen/codeplug-studio/issues/845)).
