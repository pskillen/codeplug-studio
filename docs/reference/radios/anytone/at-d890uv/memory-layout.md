...
| `RadioIdSet`                 | `0x3482c40`  | Bitmap `0x20`                                             | Radio ID occupancy. Bit **set** → occupied. Assemble-from-`0xff` **zeros** this bitmap when the projection has no IDs ([#1129](https://github.com/pskillen/codeplug-studio/issues/1129)); empty projection on a **hydrated** (non-`0xff`) bank is left alone. Shares erase unit `0x3480000` with `ChannelSet` / `ZoneSet` / `ZoneHide` — RMW skips all-`0xff` blocks, so leaving assemble fill in place made every ID slot look occupied after erase. |
...
**Sparse RMW flow (upload):** compute touched erase units from modelled write addresses → fresh-read each unit from the connected radio → identity-check LocalInfo serial against hydration → overlay modelled chunks → stage only non-`0xff` 16-byte blocks **excluding** per-unit flash sector-management markers at `+0x3fbf0` / `+0x3fff0` → transmit filtered chunks → `END` (bare `0x06` ACK; no further wire command). There is **no** separate commit command — see [flash-sectors.md](flash-sectors.md).
...
| RadioIdSet      | `0x20`  | Bit **set** → radio ID occupied. Assemble-from-`0xff` must not leave this all-set ([#1129](https://github.com/pskillen/codeplug-studio/issues/1129)). |
