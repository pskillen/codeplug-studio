...
Cite: anytone-cps `D890_MAP` for bases, **plus a hardware dump reconciled byte-for-byte against CPS `AMAir.CSV` / `AMZone.CSV` egress** (24 channels + VFO, 3 zones of 10/9/5 members, `ID890UV`). Tracked in [#756](https://github.com/pskillen/codeplug-studio/issues/756); not Read or Write in Studio today. Capacities: 256 AM channels + 1 VFO slot (index 256), 16 AM zones.
...
| `AmZoneAChannel` | `0x3884600` | `0x10`                            | A-channel = **index into that zone's member list**, confirmed: all three sampled zones read `0` while their A-channels' global indices are 18/8/4. **Element width still unconfirmed** — every sampled zone has A-channel `0`, identical under u8 and u16. anytone-cps reads u16 / writes u8; `0x10` for 16 zones only fits u8. Needs a non-first A-channel sample.                                                        |
