...
| `ScanListData`               | `0x2100000`  | Stride `0x200`; length `0xd0`                  | Scan-list records — **50** member u16s at `+0x30` (0-based global channel indices); revert channel @ `+0x94`; bytes `+0x95`…`+0xcf` pad `0xff` (empty = `0xffff`, not channel 0); priority @ `+0x2` / `+0x4`: `0xffff` = Off |
