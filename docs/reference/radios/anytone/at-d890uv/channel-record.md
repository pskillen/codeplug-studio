...
| `0x13–0x14`, `0x18`, `0x1b`, `0x1c`, `0x21`, `0x34` bit 4 (`auto_scan`), `0x44–0x63` | Contact, radio ID, scan, RX group, timeslot, auto scan, name | Re-derived (`0x1b`/`0x1c` use `0xff` when unset on fresh encode; unset RX group on RMW preserves prior). **`auto_scan` on** zone scan carriers only — Web Serial `scanAdd` on `RadioChannelDto`; membership is the scan-list member array, not this bit. |
