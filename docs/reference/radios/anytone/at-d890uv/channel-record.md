...
| `0x1c`        | Receive group list index                                                                                                                                   | `0xff` = none; values `1…` select the **1-based** receive-group list index ([receive-group-record.md](receive-group-record.md)). Unset on RMW preserves prior byte (may be `0` on legacy rows). |
| `0x1d`        | Tone2 ID index                                                                                                                                             |                                                                                                                                                                                                 |
| `0x1e`        | Tone5 ID index                                                                                                                                             |                                                                                                                                                                                                 |
| `0x1f`        | DTMF ID index                                                                                                                                              |                                                                                                                                                                                                 |
...
| `0x13–0x14`, `0x18`, `0x1b`, `0x1c`, `0x21`, `0x34` bit 4 (`auto_scan`), `0x44–0x63` | Contact, radio ID, scan, RX group, timeslot, auto scan, name | Re-derived (`0x1b`/`0x1c` use `0xff` when unset on fresh encode; unset RX group on RMW preserves prior). **Timeslot** is `0x21` **bit 0** (`0` = TS1, `1` = TS2). **SMS confirm** (`0x21` bit 1) is not modelled — fresh encode sets it (CPS default On); RMW leaves the prior bit. **`auto_scan` on** zone scan carriers only — Web Serial `scanAdd` on `RadioChannelDto`; membership is the scan-list member array, not this bit. |
