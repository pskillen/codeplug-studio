...
| `0x34` bits   | DMR CRC ignore, **auto scan** (bit 4 — "start scanning on channel select"; not scan-list membership), data ACK disable, exclude roaming, **DMO vs repeater (bit 1)**, ranging | See [byte `0x34` bit 1](#byte-0x34-bit-1-dmo-vs-repeater). Analog records often have bit 1 set — overlay only on digital DTOs.                                                                 |
...
| `0x13–0x14`, `0x18`, `0x1b`, `0x1c`, `0x21`, `0x34` bits 1 and 4 (`auto_scan`), `0x44–0x63` | Contact, radio ID, scan, RX group, timeslot, DMO/repeater, auto scan, name | Re-derived. `0x1b`/`0x1c` always write `0xff` when the DTO omits scan/RX-group. Omitted `txContactId` writes **`0xffff`** (`AT_D890_INVALID_U16`) — `0x0000` is TG slot 0, not none. **Timeslot** is `0x21` **bit 0** (`0` = TS1, `1` = TS2). **DMO vs repeater** is `0x34` **bit 1** (`1` = DMO/simplex, `0` = repeater) on digital DTOs; modelled DMO/repeater also **clear** `0x21` bits 2–3 (DCDM nibble — not CSV Repeater). **`auto_scan` on** zone scan carriers only — Web Serial `scanAdd` on `RadioChannelDto`; membership is the scan-list member array, not this bit. |
...
| Slot Suit / Work alone / unknown `0x21` bits 4/6/7           | `0`                                  | DCDM (`0x21` bits 2–3) always `0` on Write; modelled DMO/repeater clear that nibble                                                         |
...
| `0x34` except auto-scan bit 4 and DMO bit 1                  | `0`                                  | CRC ignore, data ACK, roaming, ranging Off unless `scanAdd` sets bit 4. DMO bit 1 from DTO when set; omitted → `0` (repeater polarity)      |
| R5Tone BOT / EOT `0x40` / `0x41`                             | `0`                                  |                                                                                                                                             |
| Other unknown gaps                                           | `0`                                  | Fresh `fill(0)` then overlay modelled fields                                                                                                |

## Byte `0x21`

Little-endian bit numbers. Evidence: qDMR `libdmrconf` `anytone_d868uv_codeplug.cc` (`timeSlot: {0x0021, 0}`, `smsConfirm: {0x0021, 1}`, `dmrMode` bits 2–3 as the **DCDM nibble**) plus forensic `HEALTHY_CHANNEL_RECORDS` (181 occupied slots: bit 1 always set, bit 0 set on 18; **bits 2–3 always 0**, including digital repeaters). anytone-cps `Channel::encode_D890UV` tests **bit 1** for timeslot — that swap is **not** used here. Do **not** map CSV `DMR MODE` `0`/`1` onto bits 2–3: writing `1` is radio menu **DCDM/Dual TS**.

| Bit | Field                                                                   | Studio overlay                                                                 |
| --- | ----------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| 0   | Time slot (`0` = TS1, `1` = TS2)                                        | Written from DTO `timeslot`                                                    |
| 1   | SMS Confirmation                                                        | Unmodelled. Write default **On**                                               |
| 2–3 | DCDM / Dual TS / TS split (`0` not DCDM, `1` Dual TS, `2`/`3` TS split) | Modelled DMO/repeater always write `0`. Radio menu items 3–4. Unmodelled DCDM. |
...
## Byte `0x34` bit 1 (DMO vs repeater)

CPS `DMR MODE` column `0`/`1` (DMO / Repeater) and radio menu items 1–2 map here — **not** to `0x21` bits 2–3. CSV `2`/`3` (DCDM) stay on the `0x21` nibble and are not modelled.

| Wire | Radio / CPS   | Forensic (CPS-written digital slots) |
| ---- | ------------- | ------------------------------------ |
| `1`  | DMO / simplex | Digital simplex: bit 1 set           |
| `0`  | Repeater      | Digital duplex: bit 1 clear          |

anytone-cps names this `dmr_mode` on `0x34` bit 1 (`1` sets the bit). Studio polarity matches CPS-written dumps: **set = DMO**. Overlay when `RadioChannelDto.dmrOperatingMode` is present (digital rows). Analog / omitted: write `0` (whole-record default).
