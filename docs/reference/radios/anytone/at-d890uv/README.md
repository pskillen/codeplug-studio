...
**Write contract (v1, WATCH-08):** Studio **replaces** channels, zones, scan lists, talk groups (including TalkgroupOrder), RX groups, operator radio IDs, and master radio ID from `assemble` + `RadioWriteProjection`. Serial Write projects the **DMR bank only** — AM airband and broadcast FM are omitted from MR slots, zones, and scan (CSV parity; radio AmAir bytes unchanged) ([#755](https://github.com/pskillen/codeplug-studio/issues/755)). Upload is fenced to an explicit **allow-list** of modelled bank extents ([#753](https://github.com/pskillen/codeplug-studio/issues/753)) — `listWriteChunks` and `atD890WriteMemory` reject anything outside that list. **LocalInfo** is Read for Radio image retain preview but **not serial-written**; pre/post sentinel reads verify `LocalInfo` and optional-settings spans stay byte-identical. MR channel encode rejects AM airband (108–137 MHz) in MR slots. Regions labelled **kept** in `writeRole` are not re-derived from the build and are not uploaded (except sentinel verify reads). Optional settings (`0x3500000` / `0x3500900` / `0x3501280`), alarm settings, `DigitalContact*`, boot/BK images, crypto, AM air (binary), roaming, and AnalogBook are **not** in the v1 Read/Write set. Use Anytone CSV egress for those — see [am-air.md](../../../export-formats/anytone/am-air.md). Export **Web Serial** shows a **What Write updates** coverage table. See [memory-layout.md](memory-layout.md).

**Operator note:** perform a **fresh Read** after CPS recovery before Write — do not Write from a stale hydration export. **`prodWriteDisabled`** ([#741](https://github.com/pskillen/codeplug-studio/issues/741)) stays until hardware re-check passes.
