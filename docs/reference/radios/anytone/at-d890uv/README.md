...
**Write contract (v1):** Studio **replaces** channels, zones, scan lists, talk groups (including TalkgroupOrder), RX groups, operator radio IDs, and master radio ID from `assemble` + `RadioWriteProjection`. Serial Write projects the **DMR bank only** — AM airband and broadcast FM are omitted from MR slots, zones, and scan (CSV parity; radio AmAir bytes unchanged) ([#755](https://github.com/pskillen/codeplug-studio/issues/755)). Regions labelled **kept** in `writeRole` are **not re-derived from the build** but may still be **uploaded** when present in the Read cache — notably **LocalInfo** is replayed verbatim on every Write. Optional settings (`0x3500000` / `0x3500900` / `0x3501280`), alarm settings, `DigitalContact*`, boot/BK images, crypto, AM air (binary), roaming, and AnalogBook are **not** in the v1 Read/Write set. Use Anytone CSV egress for those — see [am-air.md](../../../export-formats/anytone/am-air.md). Export **Web Serial** shows a **What Write updates** coverage table. See [memory-layout.md](memory-layout.md) for region facts and the kept-vs-replayed table.
