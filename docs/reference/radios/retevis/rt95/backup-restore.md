...
Write-codeplug still requires **project / session stash** (`hydrationRequiredForWrite: true`) as a gate, then overlays modelled channels onto an **in-session** full-clone read (`resolveRadioWriteImageForUpload`). Restore **does not** use that stash or that pre-write read; it replays zip clone bins via `upload()` only.
...
5. Progress copy is **Restore**, not “Writing codeplug.” Write `upload` stays “write these bytes” (no live `download()` inside it). Stash / Write gate still required.
