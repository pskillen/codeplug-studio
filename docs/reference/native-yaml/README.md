...
| `studioSchemaVersion` | integer | yes      | Must equal `STUDIO_SCHEMA_VERSION` in `src/core/models/schemaVersion.ts` (currently `10`). Imports accept `2`–`10`; v9 files without `library.scanLists` migrate on load; legacy `scanSkip` on channels migrates to `scanInclusion` on load; legacy `ssb-usb` / `ssb-lsb` mode values migrate to `ssb` + `ssbSideband`. |
...
| `scanListId`      | UUID    | Channel overrides only — FK to `library.scanLists[].id`                  |
...
3. `schemaVersion !== 1`
4. `studioSchemaVersion` not in `2`–`10` (current `STUDIO_SCHEMA_VERSION`)
