...
| Rule              | Detail                                                            |
| ----------------- | ----------------------------------------------------------------- |
| Member FKs        | Each `Scan Channel Member` name must exist in `Channel.CSV`       |
| Channel → scan FK | `Channel.CSV` `Scan List` must match a `Scan List Name` or `None` |
| Name length       | `Scan List Name` ≤ profile name limit (16 on AT-D890UV)           |
...
| `Look Back Time A[s]`      | Priority sample A                                                                                                                                     | `3.0` when unset ([#1069](https://github.com/pskillen/codeplug-studio/issues/1069) build override) |
| `Look Back Time B[s]`      | Priority sample B                                                                                                                                     | `3.0`                                                                                              |
| `Dropout Delay Time[s]`    | 0.1–5.0 s                                                                                                                                             | `3.0`                                                                                              |
| `Dwell Time[s]`            | 0.1–5.0 s                                                                                                                                             | `3.0`                                                                                              |

Studio export defaults all four timing columns to **`3.0`** s (wire `30` deciseconds on Web Serial) when build `exportSettings` omit overrides ([#1069](https://github.com/pskillen/codeplug-studio/issues/1069)). One build-level set applies to every library and zone-derived list. Per-list library modelling remains [#572](https://github.com/pskillen/codeplug-studio/issues/572). Operator CPS fixtures may still show other values (`1.0` / `3.1`). Scan Mode / priority / Revert remain fixed MVP constants — modelling readiness tracked under [#393](https://github.com/pskillen/codeplug-studio/issues/393).
