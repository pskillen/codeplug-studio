...
| `Zone Skip`       | `Channel.scanInclusion` + build override | `Yes`→skip                       | Mirrors `All Skip` from effective scan inclusion ([#772](https://github.com/pskillen/codeplug-studio/issues/772), [#783](https://github.com/pskillen/codeplug-studio/issues/783)) | Same value as `All Skip` on CSV and serial Write                                                    |
| `All Skip`        | `Channel.scanInclusion` + build override | `Yes`→skip                       | Override → library → build/format default (`scan` on OpenGD77 CSV and `radio-io-opengd77-*`) ([#803](https://github.com/pskillen/codeplug-studio/issues/803))                     | Global scan skip                                                                                    |
