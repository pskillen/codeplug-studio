...
**Build `wireName` overrides** that exceed the profile `nameLimit` are **hard-truncated** on export (never smart-shortened). See [wire-name-composition](../../../features/builds/wire-name-composition.md).
...
When zone-derived scan export is on, Studio also appends one synthetic **`{zoneName} Scan`** Analog/FM simplex row per exported scan list (export-only id `scan-carrier:{zoneId}`). That carrier is prepended as the first zone member and receives the list’s `scanListId`. Ordinary member channels stay `scanListId: 0`; see [scan-lists.md](scan-lists.md).
