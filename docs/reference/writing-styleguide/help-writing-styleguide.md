...
- **No wire detail in general help**: no CSV column names, no per-format values, no radio caps. Link out to the import/export hub or per-format reference (`docs/reference/export-formats/<format>/`).
