...
- PR adds `OPENGD77_*`, `CHIRP_*`, `DM32_*`, or other format/profile cap constants outside `src/core/import-export/formats/` or `docs/reference/export-formats/<format>/`
...
- PR adds CPS wire-mapping tables (column ↔ internal field) to `docs/features/` or `docs/reference/*.md` at the reference root — wire tables belong only under `docs/reference/export-formats/<format>/`
