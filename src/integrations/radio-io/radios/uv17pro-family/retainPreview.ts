/**
 * Retain-only shallow decoders for UV-17Pro family Radio image UI.
 */

import type { Uv17ProLayout } from './layout.ts';
import { decodeChannelRecord } from './channelCodec.ts';

export interface Uv17ProRetainPreviewRow {
  label: string;
  value: string;
}
...
function decodeVfoSummary(
  ...
  offset: number,
  label: string,
): Uv17ProRetainPreviewRow {
  if (offset + layout.vfoSize > bytes.length) {
    return { label, value: 'Not in image' };
  }
  const raw = bytes.subarray(offset, offset + layout.vfoSize);
  if (raw[0] === 0xff) {
    return { label, value: 'Empty' };
  }
  try {
    const ch = decodeChannelRecord(layout, raw, 0);
...
export function settingsRetainPreview(
  layout: Uv17ProLayout,
  bytes: Uint8Array,
): Uv17ProRetainPreviewRow[] {
...
export interface Uv17ProAncillaryRetainPreview {
  rows: readonly Uv17ProRetainPreviewRow[];
}

export function ancillaryRetainPreview(
  layout: Uv17ProLayout,
  bytes: Uint8Array,
): Uv17ProAncillaryRetainPreview {
  const rows: Uv17ProRetainPreviewRow[] = [
    decodeVfoSummary(layout, bytes, layout.vfoAOffset, 'VFO A'),
    decodeVfoSummary(layout, bytes, layout.vfoBOffset, 'VFO B'),
    { label: 'ANI', value: regionPresence(bytes, layout.aniOffset, layout.aniSize) },
    {
      label: 'PTT ID',
      value: regionPresence(bytes, layout.pttIdOffset, layout.pttIdSize),
    },
    {
      label: 'Upcode',
      value: regionPresence(bytes, layout.upcodeOffset, layout.upcodeSize),
    },
    {
      label: 'Downcode',
      value: regionPresence(bytes, layout.downcodeOffset, layout.downcodeSize),
    },
    ...layout.extraKeptRegions.map((r) => ({
      label: r.label,
      value: regionPresence(bytes, r.packedOffset, r.sizeBytes),
    })),
  ];
  return { rows };
}
