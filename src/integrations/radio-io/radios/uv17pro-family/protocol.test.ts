...
import { createSyntheticImageBase } from '../uv5r-mini/__fixtures__/syntheticImage.ts';
import { encodeChannelsIntoImage } from '../uv5r-mini/channelCodec.ts';
import {
...
class ScriptedPipe implements BytePipe {
  ...
  private enqueueReadReply(plain: Uint8Array, addr: number): void {
  ...
  async write(data: Uint8Array): Promise<void> {
    ...
    if (data[0] === 0x52 && this.readBlockQueue.length > 0) {
      ...
      this.enqueueReadReply(block.plain, block.addr);
    }
    if (data[0] === 0x57) {
      this.enqueueAck();
    }
...
describe('Uv17ProProtocol pre-write read', () => {
  it('download stores priorImage and emits Pre-write read stage', async () => {
  ...
  it('upload reports Upload stage during block writes', async () => {
    ...
    await radio.upload(memoryMapFromBytes(source), {
      onProgress: (p) => {
        if (p.stage) uploadStages.push(p.stage);
      },
    });
