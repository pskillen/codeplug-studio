...
describe('RT95 write without persisted hydration bag', () => {
  ...
  it('uploadPreparedRadioWrite succeeds without an egress hydration bag', async () => {
    const liveBytes = new Uint8Array(RT95_IMAGE_SIZE);
    liveBytes.fill(0xff);
    liveBytes[0x3200] = 0x42;
    ...
    await uploadPreparedRadioWrite(session, { ...egress, hydration: undefined }, undefined, {
      channels: [liveChannel],
    });
  ...
  it('hardware verify pending — operator to confirm drop-stash Write on RT95 VOX', () => {
    expect(true).toBe(true);
  });
});
