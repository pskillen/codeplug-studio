...
- `<main>` applies **block-only** padding (`padding-block: var(--mantine-spacing-md)`; `padding-inline: 0`). Horizontal gutters live on page modules via `--dsv2-page-padding-x` (12px at ≤48em, 32px above — [#1024](https://github.com/pskillen/codeplug-studio/issues/1024)). Editors that need to cancel shell block padding use `margin-block: calc(-1 * var(--mantine-spacing-md))`.
