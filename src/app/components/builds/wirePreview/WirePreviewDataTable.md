...
Read-only wire preview list for build entity routes. Shows library label, effective wire name (override or generated), and export-status badges. Row click opens `WirePreviewOverrideModal`. List cells show the effective name only — no Default/generated subline (edit surfaces show the suggestion).
