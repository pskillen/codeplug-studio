...
export default function AtD890WriteVerifyReport({ result, onClose }: AtD890WriteVerifyReportProps) {
  ...
  const markdown = useMemo(
    () =>
      formatAtD890WriteVerifyMarkdown(result, {
  ...
  async function handleCopyMarkdown(): Promise<void> {
    ...
    appendLog('Copied markdown to clipboard');
  }
    ...
    <Paper withBorder p="md" radius="md" style={{ maxHeight: '85vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <ScrollArea style={{ flex: 1 }} type="auto">
        <Stack gap="md" pr="xs">
          ...
          <Group gap="xs">
            <Button size="xs" variant="light" onClick={() => void handleCopyMarkdown()}>
              Copy markdown
            </Button>
            {mismatches.length > 0 ? (
