...
- **AT-D890UV** (`radioTargetId === 'anytone-at-d890uv`) shows four **Scan list timing** fields under the Scan lists card (Look Back A/B, Dropout Delay, Dwell). Values persist on `build.exportSettings` and apply to every library and zone-derived `ScanList.CSV` row and Web Serial scan-list record. Empty fields export the 3.0 s default.
