...
- **Display name (internal data)** — library label; **Not exported as zone** badge when `omitFromExport`; optional **displayDetails** sub-lines (e.g. DM32 RX-list fan-out: channel name, talk group name + ID + slot); fallback expansion note for other synthesized rows; **Edit in library** link.
