...
- Dual-bank (`SeparateDigitalIdList`) and D890 single-bank radios show a **Digital contacts** extra in the popup (library / RadioID directory / both / none). Source defaults **none** each open (leave contact banks unchanged). **Write contacts only** writes only that bank. Empty RadioID shadow + directory selected warns before write. OpenGD77 still rewrites the shared Contacts bank for talk groups when skipping library/RadioID extras.
