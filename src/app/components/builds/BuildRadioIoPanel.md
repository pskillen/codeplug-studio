...
- **Write Keps** (#859, design §8 Workflow B) — shown only when the egress profile has an entry in `hasSatelliteKepsWriteAdapter`/`getSatelliteKepsWriteAdapter` (`src/app/services/satelliteKepsWriteAdapters.ts`; today: AT-D890UV only). Uploads the operator's **enabled** satellites from the library (`persistence.listSatellites`) in a separate PROGRAM session from the codeplug Write — does **not** require a prior Read/hydration. Shares this panel's `busy`/`operation` state with Read/Write, so it mutually disables with them (design §9 "disable the adjacent button" — same-tab serial-lock mechanism; no separate lock primitive).
  ...
  - **Write preview** (#1074) — a collapsible "Preview satellites to write" `Panel` (defaults collapsed) shown whenever the egress profile has an entry in `getSatelliteKepsWritePreview` (today: AT-D890UV only). Live-loads the project's **enabled** satellites (`persistence.listSatellites`, filtered) via its own `useEffect`, re-running on `activeProjectId` change and on any persisted change for the active project (so toggling a satellite's `enabled` flag elsewhere in the Library updates the table without a page reload). Renders one `DataTable` row per resolved `(satellite, transmitter)` pair — satellite name, encoded (8-byte-truncated) name, mode, uplink, downlink — via `previewSatelliteWriteRecords`, the same resolution `writeSatellitesToRadio` uses, minus the wire-byte encoding. No session or write is required to see it. The "Encoded name" cell shows an `IconAlertTriangle` (orange, with a tooltip) next to the encoded value whenever the entry's `nameTruncated` flag is `true` — i.e. the 8-byte value dropped characters relative to the satellite's full name (and transmitter label, when the name left room) — added in #1075 alongside the name-first budget-allocation fix in `encodeName()`.
