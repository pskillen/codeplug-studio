...
| `cutawayEnabled`     | `boolean`                 | Optional, default **false**. When on, clips shell meshes along a vertical plane through the transmitter at `sliceBearingDeg` (`renderer.localClippingEnabled` via `globeRef.current.renderer()`, `clippingPlanes` on the existing Fresnel `MeshBasicMaterial`). Off clears planes to `[]`. Does not recreate materials.               |
| `sliceBearingDeg`    | `number`                  | Optional, default **0**. Same bearing as `SlicePlaneResult.bearingDeg`. The page passes antenna heading until the slice-plane picker has been used.                                                                                                                                                                                   |
| `rayCorridorEnabled` | `boolean`                 | Optional, default **false**. When on, adds one `customLayerData` entry (`kind: 'ray-corridor'`) rendered as `TubeGeometry` / `CatmullRomCurve3` in `MODE_COLORS[mode]`. The thin `pathsData` ray line stays visible.                                                                                                                  |
...
- **Display controls** (`display` prop):
  ...
  - **Cutaway plane** — Display toggle, default **off**. Clips ionospheric shells with a `THREE.Plane` whose normal is `cutawayPlaneNormal` (core, no three.js types) along `sliceBearingDeg`. `WebGLRenderer.localClippingEnabled` is set once on `globeRef.current.renderer()`. Planes attach to the existing shell `MeshBasicMaterial.clippingPlanes`; toggling off assigns `[]`. `customThreeObjectUpdate` stays data-only (no material/mesh recreation).
  - **Ray corridor** — Display toggle, default **off**. Extra `customLayerData` entry (`kind: 'ray-corridor'`) built with `TubeGeometry` + `CatmullRomCurve3` in `MODE_COLORS[mode]`. Additive: the thin phase-9 `pathsData` line remains.
...
- **Data:** daytime altitude bands are D 60–90 km, E 90–150 km, F1 150–250 km, F2 250–400 km. On the night hemisphere F2's lower bound drops to **150 km** (merged F-region) while D/F1 fade out. TX-local `computeIonosphericLayers` still sets `active` / `peakElectronDensity` / F2 bounds for the Reading panel and later ray tracing. `customThreeObjectUpdate` remains data-only; Fresnel + day/night live in `MeshBasicMaterial.onBeforeCompile` (`hf-shell-fresnel-daynight`). Cutaway targets those materials' `clippingPlanes`, not a custom ShaderMaterial.

## Testing

`react-globe.gl` needs a WebGL context jsdom doesn't provide, so `HfPropagationGlobe.test.tsx` mocks it to a stub component and asserts the `customLayerData`/`customThreeObject`/`customThreeObjectUpdate`/`pathsData`/`pointsData` props it receives (including that operator-hidden layers are filtered out, that an environment instant keeps D/F1 for spatial shading and adds night-shade, that terminator-on adds the sun marker plus a greyline path, that `rays` add mode-coloured paths, a skip-zone ring, and a transmitter point at `txLat`/`txLon`, that cutaway does not add extra custom-layer entries, and that ray-corridor adds a `kind: 'ray-corridor'` entry while keeping thin `pathsData` rays). Radius / path-mapping / clipping-plane / corridor-mesh math lives in `buildGlobeData.test.ts`. Dash fractions: `globePathDash.test.ts`. Terminator / subsolar math: `solarTerminator.test.ts`. Cutaway normal: `cutawayPlane.test.ts`. Ring geometry: `footprint.test.ts`. Debounced Worker fetch: `usePropagationRayTrace.test.ts`.
