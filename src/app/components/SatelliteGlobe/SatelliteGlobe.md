...
- **Orbit trails:** `orbitTrail.ts#computeGlobeOrbitTrail` — wall-clock minute windows (`lookBehindMin` / `lookAheadMin`) ahead and behind an anchor instant fixed at mount. Trail paths use sampled WGS84 altitude at each step (`sampleOrbitTrack`), converted to globe-radius units via `globeAltitude.ts`. Future trails draw solid with a sat-colour→gray fade; past trails use short repeating dashes with gray→sat-colour fade (`pathColor` gradient arrays). Whole-path dash ratios are avoided — they hide most of the look-behind segment.
