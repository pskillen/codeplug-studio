...
- **AOS/LOS clocks:** both local and UTC render for each, always 24-hour (`formatLocalClockTime`/`formatUtcClockTime` in `passTime.ts`) — never 12-hour AM/PM, which previously produced invalid-looking zero-padded output like "04:49:52 PM".
