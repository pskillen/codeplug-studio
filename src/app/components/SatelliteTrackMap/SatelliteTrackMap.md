...
- With no `selectedPass`, draws every entry in `defaultPasses` (typically the next pass per filtered satellite). The hint overlay appears only when both `selectedPass` and `defaultPasses` are empty.
