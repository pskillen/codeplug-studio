...
- **Visibility (halo/casing):** both the solid AOS→LOS line and the dashed approach line are drawn as two stacked `Polyline`s — a wider, neutral dark "casing" stroke (`rgba(0, 0, 0, 0.5)`/`0.45`, matching `dashArray` for the dashed line) underneath the per-satellite coloured line. Alpha-blending a light `colorForNoradId` hue (pale yellow/green) onto the light OSM basemap can't produce enough contrast on its own regardless of the value chosen; the casing provides hue-independent contrast instead.
