...
export interface GettingStartedContentProps {
  /** Inline empty-home panel: shorter headings, no duplicate title block. */
  compact?: boolean;
}

export default function GettingStartedContent({ compact = false }: GettingStartedContentProps) {
  return (
    <Stack gap={compact ? 'sm' : 'lg'}>
      ...
      <Stack gap="sm">
        {compact ? null : <Title order={4}>From blank to export</Title>}
      ...
      {compact ? null : (
        <>
          <Stack gap="sm">
            ...
            <Text size="sm">
              After you create a project, open <strong>Channels</strong> →{' '}
              <strong>Add from…</strong>. Search, tick what you need, and add. That saves you from
              retyping frequencies, tones, and talk-group IDs from a spreadsheet.
            </Text>
            ...
            <Text size="sm" c="dimmed">
              You can still add or edit channels by hand when a directory does not cover what you
              need.
            </Text>
          </Stack>
          ...
          <Stack gap="sm">
            ...
            <List size="sm" spacing="sm">
              <List.Item>
                <strong>One channel, several modes</strong> — A single library channel can carry
                more than one mode (for example FM and DMR on the same site). On export, Studio
                splits into the rows your radio needs — unless that radio can keep a dual-mode
                channel as one row.
              </List.Item>
              <List.Item>
                ...
                per–talk-group copies your CPS expects. You curate the site once and the talk-group
                set once.
              </List.Item>
              <List.Item>
                <strong>Scan lists from zones</strong> — If your radio keeps <strong>zones</strong>{' '}
                and <strong>scan lists</strong> separate, you can drive scan membership from zone
                membership on the format build instead of maintaining two unrelated lists. If the
                radio treats the zone as the scan list, you do not need a second structure.
              </List.Item>
            </List>
          </Stack>
